...
const Offers = () => {
  ...
  const applyFilters = (filters, ads) => {
    ...
    if (filters["All Stores"]?.label) {
      filtered = filtered.filter(ad => 
        ad.storeId?.storeName === filters["All Stores"].label
      );
    }
    ...
    if (filters["All Categories"]?.label) {
      filtered = filtered.filter(ad => 
        ad.storeId?.category === filters["All Categories"].label
      );
    }
    ...
    if (filters["All Districts"]?.label) {
      filtered = filtered.filter(ad => 
        ad.storeId?.district === filters["All Districts"].label
      );
    }
    ...
    if (filters["All Locations"]?.label) {
      filtered = filtered.filter(ad => 
        ad.storeId?.location === filters["All Locations"].label
      );
    }
    ...
    if (filters["All Offer Types"]?.label) {
      filtered = filtered.filter(ad => 
        ad.offerType === filters["All Offer Types"].label
      );
    }
              ...
              {filteredAds.length === 0 ? (
                ...
                <>
                  <div className="text-[14px] text-gray-600 mb-4 text-right">
                    Total Offers: {filteredAds.length}
                  </div>
